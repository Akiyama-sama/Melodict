  ...
  return (
    <div className="animate-fadeIn w-full rounded-lg bg-white/5 p-4 shadow-lg ring-1 ring-white/10 backdrop-blur-sm md:p-6">
      <div className="flex flex-col gap-3">
        {/* 单词本体 */}
        <h1 className="text-4xl font-bold tracking-tight md:text-5xl">{surface_form}</h1>
          ...
          <div className="flex flex-col items-start gap-1 text-lg">
            {reading && (
              <div className="flex items-baseline gap-2">
                <span className="text-xs font-semibold uppercase tracking-wider">読み</span>
                <span>{reading}</span>
              </div>
            )}
            {romaji && (
              <div className="flex items-baseline gap-2">
                <span className="text-xs font-semibold uppercase tracking-wider">Romaji</span>
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2">
          {basic_form && basic_form !== surface_form && (
            <>
              <span className="justify-self-end pt-1 text-sm font-medium">基本形:</span>
              <span className="text-base">{basic_form}</span>
            </>
          )}
          {pos && (
            <>
              <span className="justify-self-end pt-1 text-sm font-medium">品詞:</span>
